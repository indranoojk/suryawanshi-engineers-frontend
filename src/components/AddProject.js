...
const AddProject = () => {
  ...
  const handleAddProject = async (e) => {
    ...
    try {
      ...
      const addProjectResponse = await fetch(`${baseUrl}/api/project/addproject`, {
        method: "POST",
        headers: {
          Accept: "application/json",
          "Content-Type": "application/json",
      ...
      const addProjectData = await addProjectResponse.json();
    ...
    setProjectDetails({ title: "", description: "", content: "", imageUrl: "" })
    setFile(null);
    // setImageUrl("");
  ...
  return (
    <>
      ...
      <form onSubmit={handleAddImage}>  {/* Attach event handler to form submission */}
      ...
      <form onSubmit={handleAddProject}>  {/* Attach event handler to form submission */}
        <div className="p-5 text-white lg:flex pb-10" style={{ backgroundImage: `url(${'https://images.pexels.com/photos/19453624/pexels-photo-19453624/free-photo-of-high-line.jpeg?auto=compress&cs=tinysrgb&w=1280'})`, objectFit: 'contain' }}  >
          ...
          <div className="m-4 lg:m-10 lg:ml-20 lg:grid lg:grid-cols-2">
            ...
            <div className="mb-3 lg:m-2 row-span-2">
              <label htmlFor="imageUrl" className="text-xs lg:text-base font-semibold text-[#dad1bf]"> Image* </label>
              <input type="text" className="input-class mt-0 block w-full lg:w-80 px-0.5 bg-transparent border-0 border-b-2 border-[#716c6a] focus:ring-0 focus:border-gray-400" id="imageUrl" name="imageUrl" value={projectDetails.imageUrl} onChange={changeHandler} required />
            </div>
            <div>
              <button disabled={projectDetails.title.length < 3 || projectDetails.description.length < 5 || projectDetails.content.length < 15 || projectDetails.imageUrl.length < 5} type="submit" className="bg-green-500 disabled:bg-gray-500 disabled:text-gray-400 ml-20 lg:ml-20 px-12 lg:px-16 py-3 border-2 border-[#716c6a] shadow-sm hover:shadow-xl shadow-[#f5f2f2] focus:outline-none focus:shadow-outline-blue">
                SUBMIT
              </button>
            </div>
          </div>
